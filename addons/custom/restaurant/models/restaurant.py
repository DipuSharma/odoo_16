...
class RestStaff(models.Model):
    ...
    mobile = fields.Char(
        string="Mobile", size=10, track_visibility="always", required=True
    )
    ...
    staff_line_ids = fields.One2many(
        "rest.staff.lines", "connecting_field", string="Staff Line"
    )
    ...
    def create(self, vals):
        if vals.get("seq_num", _("New")) == _("New"):
            vals["seq_num"] = self.env["ir.sequence"].next_by_code(
                "rest.seq.staff"
            ) or _("New")
    ...
    def age_calculate(self):
        for rec in self:
            if rec.dob:
                print("______________________onchange_api________________________call")
                today = date.today()
                staff_dob = rec.dob
                age = (
                    today.year
                    - staff_dob.year
                    - ((today.month, today.day) < (staff_dob.month, staff_dob.day))
                )
                rec.age = age
